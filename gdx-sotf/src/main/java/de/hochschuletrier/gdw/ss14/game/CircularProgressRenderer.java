...
/** @author Xoppa */
public final class CircularProgressRenderer {
	...
	private int draw = 0;
	private final float angleOffset = 270f;
	private float originX, originY;
	private float scaleX = 1f, scaleY = 1f;
	
	public CircularProgressRenderer(final TextureRegion textureRegion) {
	...
	public CircularProgressRenderer(final Texture texture) {
	...
	public void draw(final Batch batch, final float x, final float y, float width, float height, final float angle) {
		...
		calculate(x - width/2, y - height/2, width, height, angle, u1, v1, u2, v2);
